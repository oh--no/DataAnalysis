...
def process_file(f):
    ...
    # with open("{}/{}.html".format(datadir, f), "r") as html:
    with open('FL-ATL.html','r') as html:
        soup = BeautifulSoup(html,'lxml')
        row = soup.find_all('tr',class_='dataTDRight')
        for tr in row:
            td = tr.find_all('td')
            if td[1].string.find('TOTAL') == -1: # and td[0].string.find('TOTAL') == -1:
